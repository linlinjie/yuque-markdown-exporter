...
import { pageLooksLikeSheet, readPageDocMeta } from './yuque-doc.js';

export { fetchPageDocument } from './yuque-doc.js';
...
export function inspectStructuredPage(doc = document) {
  ...
  const isSheet = pageLooksLikeSheet(doc);
  const isDataTable =
    (hasTableView && hasStructuredView) ||
    meta?.type === 'Table' ||
    meta?.format === 'laketable';
  const kind = isSheet ? 'sheet' : isDataTable ? 'table' : null;
...
function readCellValue(cell) {
  ...
  const badges = [
    ...(cell.querySelectorAll?.(
      '[role="listitem"], [data-value], [class*="tag-item"], [class*="select-item"], [class*="mention-tag"]'
    ) ?? [])
  ]
...
const LAKETABLE_ROOT_SELECTOR =
  '.lakex-table-view-container, .lakex-viewer-grid';
const LAKETABLE_HEADER_COLUMN_SELECTOR = '.lakex-viewer-grid-header-column';
const LAKETABLE_HEADER_NAME_SELECTOR = '.lakex-viewer-grid-header-column-name';
const LAKETABLE_LIST_ROW_SELECTOR = '.lakex-viewer-grid-list-row';
const LAKETABLE_ROW_INDEX_SELECTOR = '.lakex-viewer-grid-row-index';
const LAKETABLE_CELL_SELECTOR = '.lakex-viewer-grid-cell';
...
export function extractLaketableColumns(doc = document) {
  const headers = [
    ...(doc.querySelectorAll?.(LAKETABLE_HEADER_COLUMN_SELECTOR) ?? [])
  ];
  const usedNames = new Map();
  return headers
    .map((header) =>
      normalizeText(
        header.querySelector?.(LAKETABLE_HEADER_NAME_SELECTOR)?.innerText ??
          header.querySelector?.(LAKETABLE_HEADER_NAME_SELECTOR)?.textContent ??
          header.innerText ??
          header.textContent
      )
    )
    ...
    .map((name, index) => ({
      id: `column-${index}`,
      name: uniqueColumnName(name, usedNames)
    }));
}

export function extractLaketableRecords(doc, columns) {
  const records = [];
  for (const row of doc.querySelectorAll?.(LAKETABLE_LIST_ROW_SELECTOR) ?? []) {
    const index = normalizeText(
      row.querySelector?.(LAKETABLE_ROW_INDEX_SELECTOR)?.innerText ??
        row.querySelector?.(LAKETABLE_ROW_INDEX_SELECTOR)?.textContent
    );
    if (!/^\d+$/.test(index)) {
      continue;
    }
    const cells = [...(row.querySelectorAll?.(LAKETABLE_CELL_SELECTOR) ?? [])];
    const values = {};
    columns.forEach((column, columnIndex) => {
      values[column.id] = readCellValue(cells[columnIndex]);
    });
    records.push({
      key: `row-${index}`,
      values
...
async function waitForLaketableColumns({
  doc,
  sleep,
  now,
  timeoutMs = 10_000
  ...
  if (!doc.querySelector?.(LAKETABLE_ROOT_SELECTOR)) {
    return [];
  }
  ...
  for (let attempt = 0; attempt < 80; attempt += 1) {
    ...
    await sleep();
    const next = extractLaketableColumns(doc);
    if (next.length > 0) {
      return next;
    }
  }
  return extractLaketableColumns(doc);
}
...
function rowCells(row) {
  ...
  const outerCells = uniqueCells.filter(
    (cell) =>
      !uniqueCells.some(
        (candidate) =>
          candidate !== cell && containsElement(candidate, cell)
      )
  );
  if (outerCells.length === 1) {
    const outerCell = outerCells[0];
  ...
  return outerCells.length > 0 ? outerCells : directChildren(row);
}
...
export function describeDocumentShape(doc) {
  ...
  const lakexColumns =
    doc.querySelectorAll?.(LAKETABLE_HEADER_COLUMN_SELECTOR)?.length ?? 0;
  const lakexRows =
    doc.querySelectorAll?.(LAKETABLE_LIST_ROW_SELECTOR)?.length ?? 0;
  const lakexCells = doc.querySelectorAll?.(LAKETABLE_CELL_SELECTOR)?.length ?? 0;
  const lakex =
    lakexColumns || lakexRows || lakexCells
      ? `lakex=${lakexColumns}c/${lakexRows}r/${lakexCells}cells`
      : null;
  return [`tables=${tables.length}`, lakex, ...tableShapes]
    .filter(Boolean)
    .join(';')
    .slice(0, 12_000);
}
...
function layoutHasCollapsedHeader(layout) {
  return (
    layout.columns.length === 1 &&
    /[\r\n]/.test(String(layout.columns[0]?.name ?? ''))
  );
}
...
export async function captureStructuredTable({
  ...
  try {
    if (inspection.kind !== 'sheet' && originalView !== '表格视图') {
      if (!tableTab) {
        throw new TableCaptureError(
          'TABLE_VIEW_NOT_FOUND',
          '未找到语雀表格视图，请刷新页面后重试'
        );
      }
    ...
    const laketableColumns = await waitForLaketableColumns({ doc, sleep, now });
    const layout = laketableColumns.length
      ? {
          columns: laketableColumns,
          panes: [
            {
              element:
                doc.querySelector?.('.lakex-viewer-grid table') ||
                doc.querySelector?.('table') ||
                doc.querySelector?.(LAKETABLE_ROOT_SELECTOR)
            }
          ].filter((pane) => pane.element),
          rows: [{ sources: [] }],
          diagnostic: describeDocumentShape(doc),
          laketable: true
        }
      : hasGeometryTableRoot(doc)
        ? await waitForLayout({ doc, sleep, now })
        : null;
    if (layout && layoutHasCollapsedHeader(layout)) {
      throw new TableCaptureError(
        'INCOMPLETE_COLUMNS',
        '未能完整识别表格字段，请刷新页面后重试',
        { diagnostic: layout.diagnostic }
    ...
    const records = await collectVirtualRecords({
      ...
      sample: () => {
        ...
        if (layout?.laketable) {
          return extractLaketableRecords(doc, columns);
        }
        if (layout) {
