...
function describeTableShape(table) {
  const rows = tableRows(table);
  const samples = rows
    .slice(0, 4)
    .map((row, index) => `r${index}=${describeElementShape(row)}`);
  return [`rows=${rows.length}`, ...samples].join(';').slice(0, 800);
}
...
export async function captureStructuredTable({
  ...
  try {
    ...
    if (records.length === 0) {
      throw new TableCaptureError(
        'NO_RECORDS',
        `未读取到可导出的表格记录。诊断：${describeTableShape(table)}`
      );
    }
