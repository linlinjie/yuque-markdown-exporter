...
export async function fetchYuqueDocJson(fetchImpl, docApiUrl) {
  let response;

  try {
    response = await fetchImpl(docApiUrl, {
      credentials: 'include',
      cache: 'no-store',
      headers: {
  ...
  try {
    ...
    throw new ExportError('EMPTY_MARKDOWN', '未读取到语雀文档数据', { cause });
  }

  const data = json?.data && typeof json.data === 'object' ? json.data : json;
  if (!data || typeof data !== 'object') {
    throw new ExportError('EMPTY_MARKDOWN', '未读取到语雀文档数据');
  }
  return data;
}
