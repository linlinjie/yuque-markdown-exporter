...
const ORDINARY_DOC_TYPES = new Set(['Doc']);
const SHEET_DOC_TYPES = new Set(['Sheet', 'Table']);
const SHEET_DOC_FORMATS = new Set(['lakesheet', 'laketable']);
const ORDINARY_DOC_FORMATS = new Set(['lake', 'markdown', 'html', 'laketext']);

export const SHEET_ROOT_SELECTOR = [
  '.lake-sheet-canvas-container',
  '.lake-sheet-canvas',
  '[class*="lake-sheet-canvas"]'
].join(', ');
...
export function isOrdinaryYuqueDoc(meta) {
  if (!meta) {
    return false;
  }
  if (SHEET_DOC_TYPES.has(meta.type) || SHEET_DOC_FORMATS.has(meta.format)) {
    return false;
  }
  return (
    ORDINARY_DOC_TYPES.has(meta.type) || ORDINARY_DOC_FORMATS.has(meta.format)
  );
}

export function pageLooksLikeSheet(doc = document) {
  const meta = readPageDocMeta(doc);
  if (isOrdinaryYuqueDoc(meta)) {
    return false;
  }
